...
```
* Bd loads of post-treatment samples is lower than that of pre-treatment samples 
...
```
* Minimal zero-inflation, m1 is best
...
### Figure X: Boxplots of Bd loads relative to survey date, colored relative to whether collected during survey or during Bd treatment.
...
treasure_progress_plot_colored_boxes <- 
  ...
  stat_n_text(y.pos = -.75, vjust=0, size=3, y.expand.factor = 0) + # places sample sizes along x-axis in space between lowest values and axis line
  theme_classic() +
  xlab("Survey Date") + 
...
treasure_progress_plot_colored_boxes
ggsave(here::here("treasure-progress-plot-colored-boxes.png"), width = 15, height = 10, units="cm")
```

### Figure XX. Trajectories of loads on individual frogs across treatment and follow up survey.
...
multicapture_loads_plot
ggsave(here::here("out", "treasure-multicapture-loads.png"), width = 15, height = 10, units="cm")
```
