...
```
* Bd load data are highly left-skewed and appears zero-inflated. 
...
* Addition of shape predictor to m2a indicated that predictor was relatively unimportant. Model m2a is best fit. 
...
d %>%
  ...
  ggplot(aes(x = expt_trt, y = log10(jliv_ge + 1), fill = expt_trt)) + 
    ...
    labs(fill = "Frog group", 
         x = "Frog group",
         y = expression(~italic("J. lividum")~"concentration"~(log[10]~("genomic equiv. + 1")))) + 
    stat_n_text(size = 3) +
...
```{r}
m4b <- brm(bf(bd_load ~ days * expt_trt, shape ~ expt_trt),
          data = d3a,
          family = zero_inflated_negbinomial())
summary(m4b)
plot((m4b), ask = FALSE)
...
```
* expt_trt not an important predictor of shape (based on CI including 0). m4a is best. 
...
```{r}
ggsave(here::here("out",  "figures", "dusy-2012-recapture.png"), width = 15, height = 10, units = "cm")
```
