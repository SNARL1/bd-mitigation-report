...
anno_plot1_control <- d_plot %>% 
  filter(basin == "barrett" & treatment == "control" & date_label1 != "6") %>% 
  count(date_label1, treatment) %>% 
  mutate(y_value = -0.2) %>% as_tibble()

anno_plot1_treated <- d_plot %>% 
  filter(basin == "barrett" & treatment == "treated" & date_label1 != "6") %>% 
  count(date_label1, treatment) %>% 
  mutate(y_value = -0.2) %>% as_tibble()

anno_plot2_control <- d_plot %>% 
  filter(basin == "dusy" & treatment == "control" & date_label1 != "407-408" & date_label1 != "6") %>% 
  count(date_label1, treatment) %>% 
  mutate(y_value = -0.2) %>% as_tibble()

anno_plot2_treated <- d_plot %>% 
  filter(basin == "dusy" & treatment == "treated" & date_label1 != "407-408" & date_label1 != "6") %>% 
  count(date_label1, treatment) %>% 
  mutate(y_value = -0.2) %>% as_tibble()
```
...
d_plot1 <- d_plot %>%
  ...
  ggplot(aes(x = date_label1, y = log10(bd_load + 1), fill = treatment)) +
    ...
    geom_text(data = anno_plot1_control, aes(x = date_label1, y = y_value, label = n), hjust = 1.3, vjust = 1.5, size = 3) +
    geom_text(data = anno_plot1_treated, aes(x = date_label1, y = y_value, label = n), hjust = -0.2, vjust = 1.5, size = 3) +
    annotate("text", x = "6", y = -0.2, label = "139", vjust = 1.5, size = 3) +
    theme_classic() +
...
d_plot2 <- d_plot %>%
  ...
  ggplot(aes(x = date_label1, y = log10(bd_load + 1), fill = treatment)) +
    ...
    labs(title = "(b)",
      ...
      y = (expression(Bd~load~(log["10"]~copies+1)))) +
    scale_fill_manual(values=c("forestgreen","deepskyblue2")) + 
    geom_text(data = anno_plot2_control, aes(x = date_label1, y = y_value, label = n), hjust = 1.3, vjust = 1.5, size = 3) +
    geom_text(data = anno_plot2_treated, aes(x = date_label1, y = y_value, label = n), hjust = -0.2, vjust = 1.5, size = 3) +
    annotate("text", x = "6", y = -0.2, label = "121", vjust = 1.5, size = 3) +
    theme_classic() +
