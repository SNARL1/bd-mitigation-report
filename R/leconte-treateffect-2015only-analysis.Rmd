...
# plot single parameter, make changes to default plot
p1 <- conditional_effects(m2, "location:trt_period")
...
```{r}
d3 <- d %>% filter(trt_period == "pretreat") %>% 
...
```{r}
m3 <- brm(startload ~ trt_died * location,
          data = d3,
          family = negbinomial())
summary(m3)
plot(m3)
...
```{r}
m3b <- brm(bf(startload ~ status*location, 
             shape ~ status + location),
          data = d3,
          family = negbinomial())
summary(m3b)
plot(m3b)
...
loo(m3, m3b)
```
* m3b has a better fit (diff ratio = 3.1), but see LOO-PIT results.
...
```{r}
m3a <- brm(startload ~ status*location,
          data = d3,
          family = zero_inflated_negbinomial())
summary(m3a)
plot(m3a)
