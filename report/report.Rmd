---
title: Effectiveness of antifungal treatments conducted during chytridiomycosis epizootics in populations of an endangered frog
author: 
  - Roland A. Knapp^1,2^
  - Maxwell B. Joseph^3^
  - Thomas C. Smith^1,2^ 
...
*Keywords*: amphibian chytrid fungus, *Batrachochytrium dendrobatidis*, wildlife disease, zoonosis, epizootic, host population decline, antifungal treatment
...
MYL frogs are highly susceptible to chytridiomycosis, and the arrival of Bd in a naive population typically results in rapid increases in Bd prevalence and infection intensity ("load"), and subsequent mass frog die-offs [@vredenburg2010dynamics]. 
...
This suggests that antifungal treatments could work in concert with adaptive immunity to increase frog survival during epizootics and facilitate the transition of affected populations to an enzootic state. 
...
Immediately following capture, a skin swab sample was collected from all animals or a subset (depending on the treatment) to describe Bd load. Adults and subadults were measured and weighed, and adults were tagged using 8 mm passive integrated transponder (PIT) tags inserted under the dorsal skin via a small incision. 

Animals assigned to the "treated" group were held in large mesh pens (2 m * 2 m * 0.75 m) for the duration of the multi-day treatment period. 
...
Analysis of treatment results utilized linear simple and multilevel models analyzed in a Bayesian framework using R software v. 4.0.3 [@rsoftware2020]. All analyses except one used the *brms* package [@bürkner2017rpackage; @bürkner2018advanced]. 
The exception was the analysis of the CMR data collected as part of the itraconazole treatment in LeConte Basin. 
This analysis was conducted using Stan [@carpenter2017stan] directly instead of via the *brms* interface. 
...
For all models, we used *brms* defaults for priors, number of chains (4), and warmup and post-warmup iterations (1000 for each). Model convergence was evaluated using trace plots, Gelman-Rubin statistics (Rhat), and measures of effective sample size ("bulk-ESS", "tail-ESS"). 
...
To address the first question, we developed separate models to (i) describe Bd loads of the animals assigned to the treated and control groups before treatment was initiated, (ii) effectiveness of treatment in reducing Bd loads of animals in the treated group, and (iii) post-treatment loads in the treated and control groups in the year of treatment and the following year. Because the treatments in Barrett and Dusy Basins were virtually identical in their design, we combined the results from both experiments into a single dataset, and included basin as a predictor variable in models to account for any between-basin differences.
...
To evaluate any pre-treatment differences in Bd loads of frogs assigned to the treated and control groups, we used the model *bd_load ~ (location x group)* (family = negative binomial, location = upper/lower, group = treated/control). 
